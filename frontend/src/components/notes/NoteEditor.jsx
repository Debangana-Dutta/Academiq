      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="form-input resize-none h-36 font-mono text-xs leading-relaxed"
            placeholder="Write your note here..."
          />
          <p className="text-xs text-slate-400 mt-1 text-right">{content.length} / 50,000</p>
        </div>
            ...
            <div className="flex items-center gap-2 mt-1.5">
              {COLORS.map(c => (
                <button
                  ...
                  className={`w-7 h-7 rounded-full ${COLOR_DISPLAY[c]} transition-transform hover:scale-110 ${color === c ? 'ring-2 ring-offset-2 ring-slate-400 scale-110' : ''}`}
                  title={c}
                />
              ))}
            </div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {tags.map(t => (
              <span key={t} className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-slate-100 text-slate-600 rounded-full text-xs font-semibold">
                #{t}
                <button onClick={() => removeTag(t)} className="hover:text-danger-500 transition-colors">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-slate-100">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={save} disabled={saving} className="btn-primary">{saving ? 'Saving...' : note ? 'Update' : 'Create'}</button>
          </div>
        </div>
      </div>
    </Modal>
